...
import { useViewportGrid, Separator } from '@ohif/ui-next';
import { StudyBrowser } from './StudyBrowser'
import { PanelStudyBrowserHeader } from '@ohif/extension-default/src/Panels/StudyBrowser/PanelStudyBrowserHeader';
import { defaultActionIcons } from '@ohif/extension-default/src/Panels/StudyBrowser/constants';
...
function _mapDisplaySets(displaySets, displaySetLoadingState, thumbnailImageSrcMap, viewports, flaggedUIDs) {
  const thumbnailDisplaySets = [];
  const thumbnailNoImageDisplaySets = [];
  displaySets
    .filter(ds => !ds.excludeFromThumbnailBrowser)
    .forEach(ds => {
      ...
      array.push({
        displaySetInstanceUID,
        description: ds.SeriesDescription || '',
        isFlagged: flaggedUIDs.has(ds.SeriesInstanceUID), // new
        seriesNumber: ds.SeriesNumber,
        modality: ds.Modality,
        seriesDate: formatDate(ds.SeriesDate),
        numInstances: ds.numImageFrames ?? ds.instances?.length,
